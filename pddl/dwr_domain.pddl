...
;; loads an empty robot with a container held by a nearby crane
 (:action load                                
     ...
     :precondition (exists(?l - location) (and (at ?r ?l) (belong ?k ?l)
...
;; unloads a robot holding a container with a nearby crane
 (:action unload                                 
     :parameters (?k - crane ?c - container ?r - robot)
     :precondition (exists(?l - location) and (belong ?k ?l) (at ?r ?l)
                      (loaded ?r ?c) (empty ?k))
...
(:action take
  ...
  :precondition
    (and
      ...
      ;; bind the container below ?c
      (exists (?else - container)
        (on ?c ?else)))
  ...
  :effect
    (and
      ...
      ;; set new top of the pile – must rebind ?else here
      (exists (?else - container)
        (and (on ?c ?else) ; ensures we select the same one
             (top ?else ?p)))

      ;; clear all “on c X” relations for every X
      (forall (?x - container)
        (not (on ?c ?x)))))
